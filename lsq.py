...
def trigLSQ(data):
    ...
    c = lambda a : np.array([np.cos(a * data['x'][i]) for i in range(noPoints)])
    s = lambda a : np.array([np.sin(a * data['x'][i]) for i in range(noPoints)])
    ...
    F = 0
    for j, i in enumerate(alternatingRange(order)): F += sol[j] * sympify(('cos(' if i['cos'] else 'sin(') + str(i['index']) + ' * x)')
